...
const bool readLDRLight = true; // Values for the LDR analog photocell light sensor
const bool readIRSensor = true; // Arduino values for IR sensor connected to GPIO2
...
void arduinoTronSend()
{
  ...
  client.print("&lat=" + lat); // <-- GPS location in demo
...
// Arduino values for IR sensor connected to GPIO2
void readIRDetector() {
  if (!irrecv.decode(&results)) { // <-- IR VS1838 sensor connected to GPIO2
    return;
  }
  ...
  if (ircode > 0xFFFFFF) { // IR VS1838 Detector = REPEAT
    return;
  } // <-- uncommit for IR VS1838

  switchState = 10;
  unsigned int ircodekey = results.value;
  switch (ircodekey) { // Declaring IR VS1838 remote codes
    case 0xFFA25D:
      irkey = "POWER";
      switchState = 11;
      break;
